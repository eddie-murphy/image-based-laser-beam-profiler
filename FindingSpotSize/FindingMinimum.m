...
BeginPackage["FindingMinimum`"]
Needs["FindingSpotSize`FitAndZAIO`"]
(* Exported symbols added here with SymbolName::usage *)  
FindBeamMinimum::usage="FindBeamMinimum[folder,wavelength in mm] will find the beam waist and the beam waist's position for a folder of images and a textfile. Will return {whfit,wwfit}"
Begin["`Private`"] (* Begin Private Context *) 
FindBeamMinimum[folderpath_,wvlength_]:=Module[{folder=folderpath,fitandzlist,hserror,wserror,column1,column3,column5,whlist,wwlist,whguess,zposhguess,wwguess,zposwguess,lambda=wvlength,whfit,wwfit},
fitandzlist = Fazaio[folder];hserror = fitandzlist[[All, 2]];wserror = fitandzlist[[All, 4]];column1 = Transpose[List[fitandzlist[[All, 1]]]];column3 = Transpose[List[fitandzlist[[All, 3]]]];column5 = Transpose[List[fitandzlist[[All, 5]]]];
...
{beamwaist > 0, \[Infinity] > zpos > -\[Infinity]}}, {{beamwaist, 
     beamwaistguess}, {zbeamwaist, zposguess}}, {zpos}, 
   Weights -> 1/wserror^2, VarianceEstimatorFunction -> (1 &)]];{whfit,wwfit}
]
